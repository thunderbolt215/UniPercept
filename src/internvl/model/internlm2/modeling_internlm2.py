...
class InternLM2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        # print(output)
        # logits_fortest = output['logits']
        # for id in range(1, 3):

        #     # 输出最后一个token中预测概率 top30 的token
        #     last_logits = logits_fortest[:, -id, :].detach()  # 添加 detach() 断开计算图
        #     print(f'count -{id}:')
        #     top_logits, top_indices = torch.topk(last_logits, 10, dim=-1)
        #     print('inside')
        #     print(f"Top 10 token IDs: {top_indices[0].tolist()}")
        #     print(f"Top 10 token logits: {top_logits[0].tolist()}")
